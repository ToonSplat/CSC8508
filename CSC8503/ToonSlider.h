...
class ToonSlider
{
	private:
		...
		int									 m_SliderLevelMin;
		int									 m_SliderLevelMax;
		int									 m_SliderLevels;
		bool								 m_IsActive;
	...
	public:
		ToonSlider(Coordinates coordinates, int sliderLevelMin, int sliderLevelMax, Vector2 windowSize, float textWidth = 5.0f, float sliderHeight = 1.0f, float sliderHeadSize = 5.0f);	//Set text width to 0 if Slider text is not needed
		~ToonSlider();

		void Update(float dt);
		void HandleKeyboardAndMouseEvents();
		int GetCurrentLevel();
		void SetCurrentLevel(int volumeLevel);
